...
1. 子链采样：从工具 DAG 图中采样一条工具调用链路 $s_1 \subset G$。
2. 实例化数据库：针对 $s_1$ 中的每个工具，依次实例化工具涉及的数据库状态。
3. 任务生成：基于 $s_1$ 生成任务，包括任务描述、user profile、evaluation rubrics。
...
4. **预算动态分配**：通过 $v_{i,t}=V(\tau_i|\pi_{\theta_t}, \boldsymbol{m}_t)$ 估计任务价值，用 heap-based greedy algorithm 分配 rollout 预算。模型现在弱在哪，就多 rollout 哪种 task。
